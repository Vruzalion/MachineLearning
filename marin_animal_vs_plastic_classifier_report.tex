...
\subsection{Building Deep Learning/Machine Learning algorithms}

The problem we are trying to solve falls into the category of image processing. We tried several approaches to solve it, including some Machine Learning algorithms like k-Nearest Neighbor Classifier, SVM and a deep learning algorithm, precisely a Convolutional Neural Networks (CNN). We evaluated the performance of each of these models and finally chose the best one. 

The main challenges faced when solving the image classification problem are: viewpoint orientation, scale variation, deformation, occlusion, illumination conditions, background clutter and intra-class variation. Thus, a good model has to overcome all, if not most of these challenges.
